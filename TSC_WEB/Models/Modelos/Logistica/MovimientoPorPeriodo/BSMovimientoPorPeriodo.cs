                                                                        ...
                                                                        )
        {
            ...
            try
            {
                ...
                while (RegistrosMov.Read())
                {
                    ...
                    ObjEntMovPorPeriodo.fechamovimiento = string.IsNullOrEmpty((RegistrosMov["data_movimento"].ToString()).ToString()) ? null : (Convert.ToDateTime(RegistrosMov["data_movimento"]).Date.ToString("dd/MM/yyyy"));//(DateTime.Parse(RegistrosMov["data_movimento"].ToString())).ToString();
                    ...
                    ObjEntMovPorPeriodo.Fecha_Ins_Entrada = (string.IsNullOrEmpty(RegistrosMov["FECHA_INSER_ENTRADA"].ToString()) ? "" : (Convert.ToDateTime(RegistrosMov["FECHA_INSER_ENTRADA"]).Date.ToString("dd/MM/yyyy")));  
                    ObjEntMovPorPeriodo.Fecha_Ins_Salida = (string.IsNullOrEmpty(RegistrosMov["FECHA_INSER_SALIDA"].ToString()) ? "" : (Convert.ToDateTime(RegistrosMov["FECHA_INSER_SALIDA"]).Date.ToString("dd/MM/yyyy"))); 
